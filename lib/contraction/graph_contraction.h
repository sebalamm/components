...
class Contraction {
  ...
  private:
    ...
    void ExchangeGhostContractionMapping() {
      ...
      for (PEID i = 0; i < (PEID) send_buffers.size(); ++i) {
        if (g_.IsAdjacentPE(i)) {
          ...
          MPI_Request req;
          MPI_Isend(&send_buffers[i][0], send_buffers[i].size(), MPI_UNSIGNED_LONG_LONG, i, i+6*size_, MPI_COMM_WORLD, &req);
        };
      }
      ...
      while (recv_messages < num_adjacent_pes) {
        ...
        MPI_Status rst;
        MPI_Recv(&message[0], message_length, MPI_UNSIGNED_LONG_LONG, st.MPI_SOURCE, rank_+6*size_, MPI_COMM_WORLD, &rst);
        recv_messages++;
    ...
    void ExchangeGhostContractionEdges() { 
      ...
      std::vector<std::vector<VertexID>> messages(size_);
      ...
      for (PEID i = 0; i < size_; ++i) {
        if (i != rank_) {
          ...
          MPI_Request req;
          MPI_Isend(&messages[i][0], messages[i].size(), MPI_UNSIGNED_LONG_LONG, i, i+6*size_, MPI_COMM_WORLD, &req);
        }
      }
    ...
    GraphAccess BuildLocalContractionGraph() { 
      ...
      for (VertexID i = 0; i < num_local_components_; ++i) {
        ...
        for (VertexID j = 0; j < local_edge_lists[i].size(); j++) {
          VertexID target = local_edge_lists[i][j].first;
          PEID sender = local_edge_lists[i][j].second;
          cg.AddEdge(v, target, sender);
        }
      }
      cg.FinishConstruct();
      MPI_Barrier(MPI_COMM_WORLD);
